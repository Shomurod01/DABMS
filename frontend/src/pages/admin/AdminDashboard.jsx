  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 animate-stagger">
        <StatsCard icon="👨‍⚕️" label="Total Doctors"   value={stats?.totalDoctors    || 0} color="bg-teal-50 text-teal-600" />
        <StatsCard icon="👤"   label="Total Patients"  value={stats?.totalPatients   || 0} color="bg-blue-50 text-blue-600" />
        <StatsCard icon="📅"   label="Appointments"    value={stats?.totalAppointments || 0} color="bg-yellow-50 text-yellow-600" />
        <StatsCard icon="💰"   label="Total Revenue"   value={formatCurrency(stats?.totalRevenue || 0)} color="bg-violet-50 text-violet-600" />
      </div>
