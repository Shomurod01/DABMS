...
import { specialityIcons } from '../utils/helpers';
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="bg-white border-b border-slate-200 py-24 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          {/* Trust stats */}
          <div className="grid grid-cols-3 gap-4 sm:gap-6 max-w-lg mx-auto mt-16 animate-fade-in" style={{ animationDelay: '0.4s' }}>
            {[['200+', 'Doctors'], ['10K+', 'Patients'], ['50K+', 'Appointments']].map(([n, l]) => (
              <div key={l} className="text-center bg-slate-50/50 sm:bg-transparent p-2 sm:p-0 rounded-2xl">
                <p className="text-2xl sm:text-3xl font-display text-primary font-bold">{n}</p>
                <p className="text-muted text-xs sm:text-sm mt-1">{l}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 animate-stagger">
            {SPECIALITIES.map((s) => (
              ...
              >
                <span className="text-3xl group-hover:scale-110 transition-transform">{specialityIcons[s] || '🩺'}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-stagger">
            {[
              ...
              <div key={step} className="card text-center relative overflow-hidden group">
                <div className="absolute top-3 right-4 font-mono text-5xl font-bold text-slate-100 group-hover:text-slate-200 transition-colors select-none">{step}</div>
