  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 animate-stagger">
        <StatsCard label="Total"     value={stats?.total     || 0} color="bg-blue-50 text-blue-600" />
        <StatsCard label="Pending"   value={stats?.pending   || 0} color="bg-amber-50 text-amber-600" />
        <StatsCard label="Completed" value={stats?.completed || 0} color="bg-teal-50 text-teal-600" />
        <StatsCard label="Cancelled" value={stats?.cancelled || 0} color="bg-rose-50 text-rose-600" />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <Link to="/doctors" className="card flex items-center gap-4 hover:border-teal-200 group border border-slate-200 shadow-sm transition-all duration-300">
          <div className="w-12 h-12 rounded-2xl bg-teal-50 text-primary flex items-center justify-center group-hover:scale-110 transition-transform"></div>
          <div>
        ...
        <Link to="/patient/appointments" className="card flex items-center gap-4 hover:border-blue-200 group border border-slate-200 shadow-sm transition-all duration-300">
          <div className="w-12 h-12 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center group-hover:scale-110 transition-transform"></div>
          <div>
      ...
      <div>
        ...
        {appointments.length === 0 ? (
          <EmptyState title="No appointments yet" description="Book your first appointment with a verified doctor." action={<Link to="/doctors" className="btn-primary px-6 py-2.5 text-sm">Find Doctors</Link>} />
        ) : (
